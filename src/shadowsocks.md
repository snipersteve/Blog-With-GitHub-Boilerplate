...
然而某天出差，临时起意买了个四口的J1900软路由。到手后发现做工真好，被动散热效果就很不错，夏天也仅40多度。而且大小适中，正好可以放进弱电箱，把新三给换下来。性能上，跑分可以跑三万多，看着就舒服。继续拿K2P作AP，除次卧信号略差之外，稳定又给力。
...
实现远程访问后，在家里搞了一台蜗牛星际跑黑裙（矿渣真香）。用cloudsync来下载百度盘和Google Drive上的资源（cloudsync真是神器）。
...
clash在2020年6月27日突然把版本号拉到1.0.0[版本](https://github.com/Dreamacro/clash/releases/tag/v1.0.0)，这意味着开源时代的结束。此后clash将另开一个闭源版本，称为Premium内核，支持rule provider、tun等新功能。此举封死了clashr无脑抄代码的路子，但也堵死了开源合作的可能性。对于使用者而言，目前来看影响不大，clash的开源版本已经更新了proxy provider这个重要功能，配合subconverter进行节点更新十分方便。我在服务器docker里跑了1.0.0版本，使用良好。

我现在属于拿来主义，只会拿开源社区的东西来用；希望以后也有能力可以去提pr甚至自己写个有用的程序，也为开源社区作点贡献。

---

原本计划用Velop mesh作为AP，后来觉得实在是贵，消费降级，买了最便宜的水星mesh路由，2只的价格不到200，用起来效果也不错。除了参数上，wifi5略落后，2.5Ghz+5Ghz总共1200M的带宽也无法完全跑满千兆，但总体够用了，颜值也不错。比当年用华硕的Aimesh强多了，回头看华硕的路由器实在是有些坑。
