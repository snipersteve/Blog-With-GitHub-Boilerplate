...
把Docker、虚拟机等文件夹全部放到缓存，访问快一些，但是没有保护，一般也不会丢。影音媒体等不怎么访问的数据文件存在阵列上，有校验保护。因为unraid支持硬盘无访问自动休眠，对于只有晚上回家偶尔看看电影的用途来说，比较省电，也比较省硬盘。
![休眠](./images/standby.jpg)

跑PLEX后终于可以实现流畅的硬解，播放前缓冲约5秒左右，体验完美。实测在全盘待机情况下功耗约27W，PLEX硬解跑起来的时候约50W。
![功耗](./images/watt.jpg)
...
这是一台All in one的机器，理论上可以把所有服务跑在这台机器上，但我还是选择把rss相关服务跑在GK55那台机器上，国际访问起来更方便一些。通过这一波整合，淘汰了家里用来跑存储的蜗牛星际、用来跑下载及op的N1（小钢炮系统）以及用来当播放器的两台N1（CoreElec），实现了心理上的舒适。
