...
最终决定换成一台i3 8100T（带HD630显卡）的机器，6SATA+nvme，跑unraid后可以全面利用老的硬盘，而且支持硬盘休眠，对于只有晚上回家偶尔看看电影的用途来说，比较省电。

跑unraid后，新建一个下载文件夹，指定存放在1T的ssd（三星evo860，600T写入）上，专用来跑PT，不进缓存；以及一个媒体文件夹，排除ssd，也不进缓存。所以真正进缓存的只有个别docker（transmission/ddns）及虚拟机（openwrt），256的nvme就可以对付了。
